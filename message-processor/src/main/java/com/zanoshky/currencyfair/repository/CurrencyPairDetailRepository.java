...
public interface CurrencyPairDetailRepository extends JpaRepository<CurrencyPairDetail, CurrencyPairDetailIdentity> {
    ...
    /**
     * Custom query which gathers {@link CurrencyPairDetail} between two specified dates from internal database and performs custom sorting by
     * time_id in ascending order and currency_pair_id in ascending order.
     *
     * @param from {@link LocalDateTime} value from when the query will start to return data
     * @param to   {@link LocalDateTime} value up to when the query will end to return data
